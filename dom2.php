...
<body>
  ...
  <?php
  $post_id = $_GET['id'];
  $postQuery = "SELECT * from domiternary WHERE id=$post_id";
  $runPQ = mysqli_query($con, $postQuery);
  $post = mysqli_fetch_assoc($runPQ);
  ?>
  <div class="title about-header" style="background-image: url(images/domestictour/<?= $post['bgimage'] ?>);">
    <div class="iternary-title title">
      <h4><?= $post['tour_title'] ?></h4>
    </div>
  </div>
    ...
    <div class="iternary-data row">
      ...
      <div class="iternary-right col-lg-8">
        <div class="iternary-right-data">
          ...
          <?php
          $postQuery = "SELECT * from daydetails  WHERE tour_id=$post_id";
          $runPQ = mysqli_query($con, $postQuery);
