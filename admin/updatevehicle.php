...
<body>
    ...
    if (isset($_POST['update_tour'])) {
        $id = $_POST['id'];
        $vehicle_category = isset($_POST['category']) ? mysqli_real_escape_string($con, $_POST['category']) : '';
        $vehicle_name = isset($_POST['vehicle_name']) ? mysqli_real_escape_string($con, $_POST['vehicle_name']) : '';
        $price = isset($_POST['price']) ? mysqli_real_escape_string($con, $_POST['price']) : '';
        $features = isset($_POST['vehicle_feature']) ? $_POST['vehicle_feature'] : array();
        ...
        $id = $_GET['id'];
        $selectquery = "SELECT * FROM vehicle_details WHERE id=$id";
        $squery = mysqli_query($con, $selectquery);
        $tour = mysqli_fetch_assoc($squery);
        $selectquery2 = "SELECT * FROM vehicle_features WHERE vehicle_id=$id";
                            ...
                            <div class="contact-form">
                                <form action="<?php echo htmlentities($_SERVER['PHP_SELF']) ?>" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="form-outline mb-4">
                                        ...
                                        <input type="text" name="category" value="<?php echo $tour['vehicle_category'] ?>" class="form-control" required></input>
                                    </div>
                                    <div class="form-outline mb-4">
                                        <label for="vehicle_name">Vehicle Name</label>
                                        <input type="text" name="vehicle_name" value="<?php echo $tour['vehicle_name'] ?>" class="form-control" required></input>
                                    </div>
                                    <div class="form-outline mb-4">
                                    ...
                                    <?php foreach ($itinerary as $day) { ?>
                                        <div class="form-outline mb-4 mt-4" id="itinerary-container">
                                            ...
                                            <input type="text" name="vehicle_feature[]" value="<?php echo $day['feature'] ?>" class="form-control" required></input>
                                        </div>
                                    <?php } ?>
