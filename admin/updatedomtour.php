...
<body>
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            ...
            if (in_array($extension, $allowed_files)) {
                ...
                if ($size[0] < 2000 && $size[1] < 2000 && $_FILES['image']['size'] < 2097152) {
                    $updatequery = "UPDATE domiternary SET tour_title='$tour_name', amount='$amount', noofdays='$no_of_days', holidayctg='$category', highlights='$highlights',inclusion='$inclusion', exclusion='$exclusion', image='$image' WHERE id='$id'";
                    ...
                    if ($uquery) {
                        ...
                        if ($uquery) {
                            ...
                            for ($i = 0; $i < count($day_description); $i++) {
                                ...
                                $sql = "UPDATE daydetails SET day='$days', daytitle='$day_titles', daydesc='$description' ,tour_id='$id' ";
    ...
    <?php
            ...
            $updatequery = "UPDATE domiternary SET tour_title='$tour_name', amount='$amount', noofdays='$no_of_days', holidayctg='$category', highlights='$highlights',inclusion='$inclusion', exclusion='$exclusion' WHERE id='$id'";
            $uquery = mysqli_query($con, $updatequery);
            if ($uquery) {
                if (count($day_description) > 0) {
                    ...
                    for ($i = 0; $i < count($day_description); $i++) {
                        ...
                        $check_sql = "SELECT * FROM daydetails WHERE tour_id='$id' AND day='$days'";
                        $result = mysqli_query($con, $check_sql);

                        if (mysqli_num_rows($result) > 0) {
                            // Day already exists, update the day
                            $sql = "UPDATE daydetails SET daytitle='$day_titles', daydesc='$description' WHERE tour_id='$id' AND day='$days'";
                            mysqli_query($con, $sql);
                        } else {
                            // Day doesn't exist, insert new day
                            $sql = "INSERT INTO daydetails (tour_id, day, daytitle, daydesc) VALUES ('$id', '$days', '$day_titles', '$description')";
                            mysqli_query($con, $sql);
                        }
                    }
                }
    ...
    <?php
        ...
        $id = $_GET['id'];
        $selectquery = "SELECT * FROM domiternary WHERE id=$id";
        $squery = mysqli_query($con, $selectquery);
        $tour = mysqli_fetch_assoc($squery);
        $selectquery2 = "SELECT * FROM daydetails WHERE tour_id=$id";
                        ...
                        <div class="contact-form">
                            <form action="<?php echo htmlentities($_SERVER['PHP_SELF']) ?>" method="POST"
                                ...
                                <div class="form-outline mb-4 mt-4" id="itinerary-container">
                                    ...
                                    <textarea type="file" name="day[]" class="form-control"
                                        required><?php echo $day['day'] ?></textarea>
                                    <label for="day_title">Day Title</label>
                                    <textarea type="file" name="day_title[]" class="form-control"
                                        required><?php echo $day['daytitle'] ?></textarea>
                                    <label for="day_description">Day Description</label>
                                    <textarea type="file" name="day_description[]" class="form-control"
                                        required><?php echo $day['daydesc'] ?></textarea>
                                </div>
                                <?php } ?>
